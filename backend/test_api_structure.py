#!/usr/bin/env python3
"""
Test script to verify the new API response structure
"""

import json
from datetime import datetime

def test_api_response_structure():
    """Test the new API response structure"""
    
    # Mock response data matching the new structure
    mock_response = {
        "room_analysis": {
            "detections": {
                "walls": [
                    {"type": "wall", "color": "#f0f0f0", "area": 15.5},
                    {"type": "wall", "color": "#e8e8e8", "area": 12.3}
                ],
                "windows": [
                    {"type": "window", "area": 2.1, "position": "north"},
                    {"type": "window", "area": 1.8, "position": "south"}
                ],
                "furniture": [
                    {"name": "sofa", "type": "furniture", "color": "#8B4513"},
                    {"name": "coffee_table", "type": "furniture", "color": "#654321"},
                    {"name": "bookshelf", "type": "furniture", "color": "#2F4F4F"}
                ],
                "other": [
                    {"name": "lamp", "type": "lighting", "color": "#FFD700"},
                    {"name": "plant", "type": "decoration", "color": "#228B22"}
                ]
            },
            "color_palette": [
                {"rgb": [200, 180, 160], "hex": "#c8b4a0", "percentage": 35.2},
                {"rgb": [139, 69, 19], "hex": "#8b4513", "percentage": 25.8},
                {"rgb": [47, 79, 79], "hex": "#2f4f4f", "percentage": 20.1},
                {"rgb": [255, 215, 0], "hex": "#ffd700", "percentage": 12.5},
                {"rgb": [34, 139, 34], "hex": "#228b22", "percentage": 6.4}
            ],
            "lighting": {
                "mean_brightness": 125.5,
                "lighting_condition": "moderate"
            },
            "aesthetic_style": {
                "style": "modern minimalist interior design",
                "confidence": 0.87
            }
        },
        "recommendations": [
            {
                "artwork_id": "art_001",
                "title": "Modern Abstract",
                "match_score": 0.95,
                "reasoning": "Matches your modern style; Complements your #c8b4a0 color scheme",
                "artist": "Jane Smith",
                "price": 450,
                "image_url": "https://example.com/art1.jpg",
                "style": "modern"
            },
            {
                "artwork_id": "art_002", 
                "title": "Minimalist Lines",
                "match_score": 0.89,
                "reasoning": "Perfect for minimalist aesthetic; Neutral colors complement your space",
                "artist": "John Doe",
                "price": 320,
                "image_url": "https://example.com/art2.jpg",
                "style": "minimalist"
            },
            {
                "artwork_id": "art_003",
                "title": "Earth Tones",
                "match_score": 0.82,
                "reasoning": "Warm earth tones match your furniture; Adds natural warmth",
                "artist": "Sarah Wilson",
                "price": 280,
                "image_url": "https://example.com/art3.jpg",
                "style": "contemporary"
            }
        ],
        "trend_insights": {
            "evolution_insights": "Your modern style is evolving with current trends. The dominant trend for modern styles is color. Consider incorporating warm earth tones to stay current.",
            "trending_complements": ["warm minimalism", "biophilic design", "curved furniture"],
            "seasonal_adaptations": {
                "season": "winter",
                "suggestions": ["warm textiles", "cozy lighting", "rich colors", "layered textures"]
            },
            "analysis_timestamp": datetime.now().isoformat()
        },
        "location_suggestions": {
            "nearby_stores": [
                {
                    "name": "Modern Art Gallery",
                    "address": "123 Main St, City",
                    "rating": 4.5,
                    "types": ["art_gallery", "home_decor"],
                    "phone": "+1-555-0123",
                    "is_open": True,
                    "place_id": "ChIJ123456789",
                    "distance": 0.8
                }
            ]
        },
        "final_reasoning": "Your room has a modern minimalist interior design aesthetic with 87% confidence. The dominant color in your space is #c8b4a0. Your space has moderate lighting conditions. Our top recommendation is 'Modern Abstract' which perfectly complements your space. Based on current trends, we've also considered evolving design preferences.",
        "session_id": "session_20241201_143022",
        "success": True
    }
    
    # Test structure validation
    print("Testing API Response Structure...")
    print("=" * 50)
    
    # Check required fields
    required_fields = [
        "room_analysis", "recommendations", "trend_insights", 
        "location_suggestions", "final_reasoning", "success"
    ]
    
    for field in required_fields:
        if field in mock_response:
            print(f"✓ {field}: Present")
        else:
            print(f"✗ {field}: Missing")
    
    # Check room_analysis structure
    print("\nRoom Analysis Structure:")
    room_analysis = mock_response["room_analysis"]
    room_fields = ["detections", "color_palette", "lighting", "aesthetic_style"]
    
    for field in room_fields:
        if field in room_analysis:
            print(f"  ✓ {field}: Present")
        else:
            print(f"  ✗ {field}: Missing")
    
    # Check detections structure
    print("\nDetections Structure:")
    detections = room_analysis["detections"]
    detection_types = ["walls", "windows", "furniture", "other"]
    
    for detection_type in detection_types:
        if detection_type in detections:
            count = len(detections[detection_type])
            print(f"  ✓ {detection_type}: {count} items")
        else:
            print(f"  ✗ {detection_type}: Missing")
    
    # Check recommendations structure
    print("\nRecommendations Structure:")
    recommendations = mock_response["recommendations"]
    print(f"  ✓ Total recommendations: {len(recommendations)}")
    
    if recommendations:
        rec_fields = ["artwork_id", "title", "match_score", "reasoning"]
        for field in rec_fields:
            if field in recommendations[0]:
                print(f"  ✓ {field}: Present")
            else:
                print(f"  ✗ {field}: Missing")
    
    # Test data types
    print("\nData Type Validation:")
    print(f"  ✓ Style confidence: {type(room_analysis['aesthetic_style']['confidence'])} = {room_analysis['aesthetic_style']['confidence']}")
    print(f"  ✓ Match score: {type(recommendations[0]['match_score'])} = {recommendations[0]['match_score']}")
    print(f"  ✓ Color palette count: {len(room_analysis['color_palette'])}")
    print(f"  ✓ Total detected objects: {sum(len(detections[t]) for t in detection_types)}")
    
    print("\n" + "=" * 50)
    print("API Response Structure Test Complete!")
    
    # Save mock response to file for frontend testing
    with open("mock_api_response.json", "w") as f:
        json.dump(mock_response, f, indent=2)
    
    print("Mock response saved to mock_api_response.json")
    
    return mock_response

if __name__ == "__main__":
    test_api_response_structure()
